...
Экспорт: Сохраняет график в EPS для дальнейшего анализа.""")
    ...
    count_entry["validatecommand"] = (ctrl_frame.register(validate_digit), "%P")
    ...
    ttk.Entry(mod_frame, textvariable=mod_n, width=5, validate="key").pack(side="right", padx=(5, 0))
    ...
    ttk.Entry(rainbow_frame, textvariable=rainbow_cycle, width=5, validate="key").pack(side="right", padx=(5, 0))
    ...
    ttk.Label(spiral_params_frame, text="Плотность точек:").pack(anchor="w", padx=5, pady=(0, 5))
    ttk.Scale(spiral_params_frame, variable=spiral_density, from_=0.1, to=2.0, orient="horizontal", length=150).pack(padx=5, pady=(0, 5))
    ttk.Label(spiral_params_frame, text="Радиус спирали:").pack(anchor="w", padx=5, pady=(0, 5))
    ...
    def get_color(idx, value):
        ...
        try:
            if palette == "Радужный градиент":
                cycle = max(int(rainbow_cycle.get() or 100), 2)
                ratio = (value % cycle) / cycle
                hue = ratio * 360
                return hsv_to_rgb(hue)
            elif palette == "Mod N":
                n = max(int(mod_n.get() or 2), 2)
    ...
    def draw_columns(data):
        ...
        bar_w = max(1, (w - total_spacing) / num_bars * bar_width.get())
        spacing = 1
    ...
    def draw():
        try:
            count = max(1, int(count_var.get() or 100))
    ...
    def export_eps():
        try:
            out_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "1d_output.eps")
            canvas.postscript(file=out_path, colormode="color")
            messagebox.showinfo("Экспорт", f"График сохранён в {out_path}")
    ...
    for var in [selected_vis, selected_palette, mod_n, rainbow_cycle, scale_factor, bar_width, rotation, spiral_density, spiral_radius_factor, count_var]:
        var.trace_add("write", lambda *_: win.after(200, draw))
