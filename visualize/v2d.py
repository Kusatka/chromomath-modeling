...
from .hmm_module import model_ker_sum_squares, model_ker_sum_cubes
...
""")
    ...
    def get_color_2d(x, y, val):
        ...
        if palette == "Gradient по Ker":
            ratio = val / 9  # Нормализация 1-9
            hue = ratio * 360
            return hsv_to_rgb(hue)  # Полный HSV-градиент
        elif palette == "Mod N":
            try:
                n_val = mod_n.get()
                n = max(n_val, 2)
    ...
    def draw_model():
        ...
        for i, y in enumerate(range(ymin, ymax + 1)):
            for j, x in enumerate(range(xmin, xmax + 1)):
                val = model_func(abs(x), abs(y))  # Симметрия для отрицательных
    ...
    def export_eps():
        try:
            canvas.postscript(file="2d_output.eps", colormode='color')
            messagebox.showinfo("Экспорт", "Сетка сохранена в 2d_output.eps")
