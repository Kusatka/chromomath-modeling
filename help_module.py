...
def show_about():
    ...
    Label(header_frame, text="Хромоматематическое моделирование", font=("Arial", 18, "bold"), fg="white", bg="#2C5F8A", padx=10, pady=10).pack(fill="x")
    # Информация
    info_frame = Frame(about_window, bg="#F0F8FF")
    info_frame.grid(row=1, column=0, sticky="nsew", padx=20, pady=10)
    info_frame.grid_rowconfigure(0, weight=1)
    info_frame.grid_columnconfigure(0, weight=1)
    info_text = """Программа для хромоматематического моделирования (улучшенная версия)

Особенности:
- Динамическая генерация данных с настраиваемым количеством
- Улучшенная обработка ошибок и валидация ввода
- Полный HSV-градиент для цветов
- Поддержка отрицательных диапазонов в 2D для симметрии
- Экспорт графиков в EPS
...
"""
    ...
    Label(contact_frame, text="Контакты:  | Руководитель: Цвырко О.Л.", font=("Arial", 9), fg="#2C5F8A", bg="#E1ECF4").pack(fill="x")
    close_btn = Button(about_window, text="Закрыть", command=about_window.destroy, font=("Arial", 10), width=12, bg="#4A7B9D", fg="white", relief="flat")
...
def show_help():
    ...
    tab3 = ttk.Frame(tab_control)  # Новая вкладка для улучшений
    tab_control.add(tab1, text="Общая информация")
    tab_control.add(tab2, text="Функции моделирования")
    tab_control.add(tab3, text="Улучшения")
...
"""
    ...
    Label(tab1, text=help_text1, justify="left", font=("Arial", 10), wraplength=450).grid(row=0, column=0, sticky="nsew", padx=10, pady=10)
    # Содержимое вкладки 2
    help_text2 = """Хромоматематические модели:

1. Для кубов (1D):
   - HMM_Grad: Радужная градиентная модель (полный HSV)
   - HMM_ModN: Дискретное представление по модулю N

2. Для Ker-функции (2D):
   - HMM_Ker: Цветовое кодирование значений Ker (полный HSV)
   - HMM_ModN: Дискретное представление по модулю N
...
"""
    ...
    Label(tab2, text=help_text2, justify="left", font=("Arial", 10), wraplength=450).grid(row=0, column=0, sticky="nsew", padx=10, pady=10)
    # Содержимое вкладки 3 (улучшения)
    help_text3 = """Улучшения в версии:
- Динамическая генерация данных с вводом количества
- Обработка отрицательных значений и ошибок ввода
- Полный HSV-градиент для всех моделей
- Поддержка симметрии в 2D (отрицательные диапазоны)
- Экспорт визуализаций в EPS
- Unit-тесты для Ker-функции
"""
    tab3.grid_rowconfigure(0, weight=1)
    tab3.grid_columnconfigure(0, weight=1)
    Label(tab3, text=help_text3, justify="left", font=("Arial", 10), wraplength=450).grid(row=0, column=0, sticky="nsew", padx=10, pady=10)
    close_btn = Button(help_window, text="Закрыть", command=help_window.destroy, font=("Arial", 10), width=12, bg="#4A7B9D", fg="white", relief="flat")
